...
if ! grep danbooru /etc/passwd >/dev/null; then
    ...
    ln -s /danbooru /home/danbooru/danbooru
...
if ! grep -q "127" "/etc/postgresql/12/main/pg_hba.conf"; then
  echo "host danbooru2,danbooru2_test danbooru 127.0.0.1/32 trust" >> /etc/postgresql/12/main/pg_hba.conf
fi
