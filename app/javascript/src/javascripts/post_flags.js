...
PostFlags.initFlagForm = function () {
  ...
  function updateNoteRequired () {
    ...
    isNoteRequired = selected.attr("data-require-explanation") === "true";
  ...
  // Initial state
  isNoteRequired = form.find("input[name='post_flag[reason_name]']:checked").attr("fata-require-explanation") === "true";
  isNoteEmpty = (noteField.val() || "").trim() === "";
  noteLabel.toggleClass("required", isNoteRequired);
  toggleSubmitButton();
};
...
$(() => {
  ...
  if (Page.matches("posts-flags", "new")) PostFlags.initFlagForm();
});
