...
function showMascot(cur) {
  ...
  if (mascots[cur][2]) {$('#mascot_artist').html("Mascot by " + mascots[cur][2]);} else {$('#mascot_artist').html("&nbsp;");}
}
...
function initMascots() {
  ...
  if (isNaN(Mascots.current) || Mascots.current < 0 || Mascots.current >= mascots.length) {Mascots.current = Math.floor(Math.random() * mascots.length);}
  showMascot(Mascots.current);
}

$(function () {
  if ($('#c-static > #a-home').length) {initMascots();}
});
