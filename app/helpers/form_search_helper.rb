...
module FormSearchHelper
  def form_search(path:, always_display: false, hideable: params.fetch(:action) != "search", method: :get, policy: nil, hide_on_load: false, &)
    # dedicated search routes like /comments/search should always show
    search_params = params[:search] || {}
    show_on_load = !hideable || (!hide_on_load && (filled_form_fields(search_params, &).any? || always_display))
