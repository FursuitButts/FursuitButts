...
ORDER BY u1.id DESC, u2.last_logged_in_at DESC;")
    ...
    def user_params
      params.require(:user).slice(:profile_about, :profile_artinfo, :email, :base_upload_limit, :enable_privacy_mode, :v3_api_limit, :editor_banned, :display_name, :use_gravatar).permit([:profile_about, :profile_artinfo, :email, :base_upload_limit, :enable_privacy_mode, :v3_api_limit, :editor_banned, :display_name, :use_gravatar])
    end
  end
end
