...
BEGIN
  ...
  IF NEW.source != OLD.source
    ...
    OR NEW.is_note_locked != OLD.is_note_locked
    OR NEW.is_rating_locked != OLD.is_rating_locked
    OR NEW.is_status_locked != OLD.is_status_locked
    OR NEW.is_pending != OLD.is_pending
    OR NEW.is_flagged != OLD.is_flagged
    OR NEW.is_deleted != OLD.is_deleted
    OR NEW.approver_id != OLD.approver_id
    OR NEW.last_noted_at != OLD.last_noted_at
    OR NEW.tag_string != OLD.tag_string
    OR NEW.parent_id != OLD.parent_id
    OR NEW.has_active_children != OLD.has_active_children
    OR NEW.bit_flags != OLD.bit_flags
    OR NEW.locked_tags != OLD.locked_tags
    OR NEW.description != OLD.description
    OR NEW.bg_color != OLD.bg_color
    OR NEW.is_comment_disabled != OLD.is_comment_disabled
    OR NEW.is_comment_locked != OLD.is_comment_locked
    OR NEW.thumbnail_frame != OLD.thumbnail_frame
    OR NEW.min_edit_level != OLD.min_edit_level
    OR old_md5 != new_md5
  THEN
...
ALTER TABLE ONLY public.edit_histories
    ADD CONSTRAINT fk_rails_92d53c2439 FOREIGN KEY (user_id) REFERENCES public.users(id);
